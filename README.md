...
AdaptLearn is an intelligent quiz and flashcard system that maximizes learning retention through adaptive spaced repetition. It implements a variant of the SM-2 algorithm (used by Anki and SuperMemo) to schedule reviews at scientifically optimal intervals, ensuring learners review material just before they would forget it.

The engine adapts to each learner individually. It tracks performance history, calculates per-topic mastery percentages, adjusts question difficulty based on accuracy and self-reported confidence, and prioritizes weak areas in quiz generation. The quiz selection algorithm balances three priorities: due review cards (40%), weak topic reinforcement (30%), and difficulty-appropriate new material (30%).

AdaptLearn supports three question formats — multiple choice, fill-in-the-blank, and true/false — generated dynamically from a structured curriculum. The curriculum system supports topic prerequisites, enabling progressive learning paths where foundational topics must be mastered before advancing.

Key pedagogical features include: a motivation system with learning streaks, detailed study reports with per-topic mastery visualization, weak area identification, and personalized study recommendations. The difficulty adapter uses a sliding window over recent performance to smoothly adjust challenge level — preventing both boredom (too easy) and frustration (too hard).

The architecture is modular with five core classes: SpacedRepetitionScheduler (SM-2 algorithm), QuestionGenerator (multi-format question creation), DifficultyAdapter (performance-based adjustment), LearnerProfile (comprehensive progress tracking), and AdaptLearn (orchestrator). This design allows easy extension with new question types, curricula, or scheduling algorithms.

Real-world application: educators can define any curriculum as structured JSON, and AdaptLearn handles the rest — optimal scheduling, adaptive difficulty, progress tracking, and personalized recommendations. It works for any domain: languages, science, programming, history, or professional certification prep.
...
function main() {
  ...
  console.log(`📝 Quiz generated: ${quiz.questionCount} questions (target difficulty: ${quiz.targetDifficulty})`);
  ...
  for (let i = 0; i < Math.min(quiz.questions.length, simulatedAnswers.length); i++) {
    ...
    console.log(`\n  Q${i + 1} [${q.type}]: ${q.question.substring(0, 70)}...`);
    ...
    console.log(`    Next review in ${result.interval} day(s)`);
  }
  ...
  console.log(`  👤 Learner: ${report.learner.id}`);
  console.log(`  🔥 Streak: ${report.learner.streak} day(s)`);
  console.log(`  📊 Accuracy: ${report.learner.overallAccuracy}%`);
  console.log(`  📝 Answered: ${report.learner.totalAnswered} questions`);

  console.log(`\n  📦 Card Status:`);
  console.log(`     Total: ${report.cardStatus.total}`);
  console.log(`     Mastered: ${report.cardStatus.mastered}`);
  console.log(`     Learning: ${report.cardStatus.learning}`);
  console.log(`     New: ${report.cardStatus.new}`);
  console.log(`     Due: ${report.cardStatus.dueForReview}`);

  console.log(`\n  📈 Topic Mastery:`);
  Object.entries(report.topicMastery).forEach(([topic, data]) => {
    const bar = '█'.repeat(Math.round(data.mastery / 10)) + '░'.repeat(10 - Math.round(data.mastery / 10));
    console.log(`     ${topic}: ${bar} ${data.mastery}% (${data.correct}/${data.total})`);
  });

  if (report.weakAreas.length > 0) {
    console.log(`\n  ⚠️  Weak Areas:`);
    report.weakAreas.forEach(a => console.log(`     ${a.topic}: ${a.mastery}%`));
  }

  console.log(`\n  💡 Recommendations:`);
  report.recommendations.forEach(r => console.log(`     [${r.priority}] ${r.message}`));

  console.log('\n\n📖 HOW TO USE:');
  console.log('  const engine = new AdaptLearn();');
  console.log('  engine.loadCurriculum(topics);');
  console.log('  const quiz = engine.generateQuiz("student1", 10);');
  console.log('  const result = engine.submitAnswer("student1", conceptId, answer, confidence);');
  console.log('  const report = engine.getStudyReport("student1");');
}
